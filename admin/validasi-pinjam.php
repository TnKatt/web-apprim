...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($tanggal_pemakaian) || $tanggal_pemakaian < $current_date) {
        ...
        $date = new DateTime($tanggal_pemakaian);

        // Format tanggal menggunakan IntlDateFormatter dengan lokal Indonesia
        ...
        if ($datetime_mulai >= $datetime_selesai) {
            ...
            if (empty($error_message)) {
                ...
                $stmt_insert = $conn->prepare("
                    INSERT INTO peminjaman (kode_ruangan, tanggal_pemakaian, waktu_mulai, waktu_selesai, keperluan, nik, tanggal_peminjaman)    
                    VALUES (?, ?, ?, ?, ?, ?, DATE_ADD(CONVERT_TZ(NOW(), '+00:00', '+07:00'), INTERVAL 8 HOUR))
                ");
                $stmt_insert->bind_param("ssssss", $kode_ruang, $tanggal_pemakaian, $waktu_mulai, $waktu_selesai, $keperluan, $nik);

                if ($stmt_insert->execute()) {
                    // Ambil ID peminjaman setelah berhasil insert
                    $id_peminjaman = $stmt_insert->insert_id;

                    // Menambahkan ke tabel riwayat
                    $stmt_riwayat = $conn->prepare("INSERT INTO riwayat (nik, id_peminjaman) VALUES (?, ?)");
                    $stmt_riwayat->bind_param("si", $nik, $id_peminjaman);
                    $stmt_riwayat->execute();

                    // Menambahkan ke tabel notifikasi setelah peminjaman berhasil
            ...
            <div class="content">
                ...
                <form action="../admin/validasi-pinjam.php?id_gedung=<?= $id_gedung ?>&kode_ruangan=<?= $kode_ruang ?>" method="POST">
                    <div class="form-group">
                        ...
                        <input type="text" name="tanggal_pemakaian" id="tanggal_pemakaian" value="<?= htmlspecialchars($formatted_date) ?>" readonly required>
