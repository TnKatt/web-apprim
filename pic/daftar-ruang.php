...
if (isset($_GET['kode_ruangan']) && isset($_GET['status_ruangan'])) {
    ...
    if (isset($_GET['kode_ruangan']) && isset($_GET['status_ruangan'])) {
        ...
        if ($stmt_update->execute()) {
            ...
            header("Location: ../pic/daftar-ruang.php?id_gedung=" . $id_gedung . "&status_ruangan_update=success");
            exit();
        } else {
            // Redirect dengan pesan error
            header("Location: ../pic/daftar-ruang.php?id_gedung=" . $id_gedung . "&status_ruangan_update=failure");
            exit();
        }
    }
}
...
<head>
    ...
    <link rel="stylesheet" href="../back-end/pic/daftar-ruang.css">
            ...
            <div class="content">
                ...
                <?php if ($result_ruang->num_rows > 0): ?>
                    <?php while ($ruang = $result_ruang->fetch_assoc()): ?>
                        <div class="room-box">
                            ...
                            <div class="room-right">
                                ...
                                <div class="button-container">
                                    <?php if (isset($ruang['nik_pic']) && $ruang['nik_pic'] == $nik): ?>
                                        <!-- Jika yang login adalah PIC ruangan -->
                                        <!-- Tombol Ketersediaan -->
                                        <a href="../pic/ketersediaan.php?kode_ruangan=<?= isset($ruang['kode_ruangan']) ? htmlspecialchars($ruang['kode_ruangan']) : '' ?>" class="edit-room-btn">Ketersediaan</a>
                                        <!-- Tombol Edit Ruangan -->
                                        <a href="../pic/edit-ruang.php?id_gedung=<?= isset($gedung_data['id_gedung']) ? htmlspecialchars($gedung_data['id_gedung']) : '' ?>&kode_ruangan=<?= isset($ruang['kode_ruangan']) ? htmlspecialchars($ruang['kode_ruangan']) : '' ?>" 
                                        ...
                                        <?php if (isset($ruang['status_ruangan']) && strtolower(trim($ruang['status_ruangan'])) === 'terbuka'): ?>
                                            ...
                                            <a href="../pic/daftar-ruang.php?id_gedung=<?= $id_gedung ?>&kode_ruangan=<?= urlencode($ruang['kode_ruangan']) ?>&status_ruangan=tertutup"
                                            class="status-room-btn" style="background-color: green;">Terbuka</a>
                                        <?php else: ?>
                                            <a href="../pic/daftar-ruang.php?id_gedung=<?= $id_gedung ?>&kode_ruangan=<?= urlencode($ruang['kode_ruangan']) ?>&status_ruangan=terbuka"
                                            class="status-room-btn" style="background-color: #800e13;">
                                            Tertutup
                                            </a>
                                        <?php endif; ?>

                                    <?php else: ?>
                                        <!-- Jika yang login bukan PIC ruangan -->
                                        <!-- Tombol Ketersediaan -->
                                        <a href="../pic/ketersediaan.php?kode_ruangan=<?= isset($ruang['kode_ruangan']) ? htmlspecialchars($ruang['kode_ruangan']) : '' ?>" class="edit-room-btn">Ketersediaan</a>
                                        ...
                                        <!-- Tombol Pinjam Ruang -->
                                            <a href="pinjam-ruang.php?id_gedung=<?= htmlspecialchars($id_gedung); ?>&kode_ruangan=<?= htmlspecialchars($ruang['kode_ruangan']); ?>" class="edit-room-btn">Pinjam Ruang</a>
                                            <a class="status-room-btn" style="background-color: green;">Terbuka</a>
                                        <?php else: ?>
                                            <a class="status-room-btn" style="color: white; text-decoration: none; font-size: 14px; padding: 8px 15px; background-color: #800e13; border-radius: 5px; transition: background-color 0.3s;">
                                                Tertutup
                                            </a>
                                        <?php endif; ?>
                                    <?php endif; ?>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                    <p style="text-align: center; font-size: 18px; font-weight: bold; margin-top: 20px;">Tidak ada data ruangan yang dimasukkan.</p>
                <?php endif; ?>
            </div>
        </div>
